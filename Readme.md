...
Yet another fork of [react-snap](https://github.com/stereobooster/react-snap). Unfortunately, 
the original project is not maintained anymore, and no fork has taken over the project. 
Luckily the original is not a big project, so it's easy to maintain. If you want to use react-snap, then 
forking the original and updating the dependencies will probably be the easiest way to go.
